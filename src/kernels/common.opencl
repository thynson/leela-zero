...
__kernel void decode_features(__global unsigned short * restrict in, __global net_t * restrict out, const int in_stride, const int out_stride) {
    const int pos = get_global_id(0);
    const int seg = get_global_id(1);
    const int pos_o = pos * out_stride + seg * 16, pos_i = pos * in_stride + seg;
    for (int i = 0; i < 16; ++i) {
        int pos = pos_o + i;
